...
import "SelmerUtility.m": AllFixedFieldsOfInertia, InertiaInFixedLine, FqVecToFp;
...
intrinsic RelaxedRank(sel::ModPSelData) -> RngIntElt 
	...
	return RankFromNumberOfLines(#sel`normal_subfields, #sel`base_rep`finite_field);
end intrinsic;
...
intrinsic NearlyOrdinaryRank(sel::ModPSelData,lines::SeqEnum) -> RngIntElt
	...
	//require &and [lines[i] in &join [sub<RSpace(rho`finite_field,rho`dim) | rho`fixed_by_decomp[i][j]> : j in [1..#rho`fixed_by_decomp[i]]] : i in [1..#lines] ] : "Lines must in a space fixed by the decomposition group at each prime.";
	...
	for i in [1..#sel`normal_subfields] do 
		...
		for j in [1..#lines] do 
			...
			line := FqVecToFp(lines[j]) * sel`normal_subfields_conjugators[i];
	...
	return RankFromNumberOfLines(lines_in_NO, #rho`finite_field);
end intrinsic;
...
intrinsic UnramifiedRank(sel::ModPSelData) -> RngIntElt
	...
	SizeOfGalM := (#sel`base_rep`finite_field)^(sel`base_rep`dim);
	lines_in_unram := [v: v in sel`normal_subfields_inertia_fixed | &and[Degree(u) eq SizeOfGalM : u in v] ];

	return RankFromNumberOfLines(#lines_in_unram,#sel`base_rep`finite_field);
end intrinsic;
